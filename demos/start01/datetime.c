...
bool_t get_zipdatetime(TZipDateTimeDef* zdt)
{
	TDateTime dt;
...
bool_t set_zipdatetime(TZipDateTimeDef* zdt)
{
	TDateTime dt;
...
bool_t get_datetime(TDateTime* dt)
{
	if(rtc_dev){
...
bool_t set_datetime(TDateTime* dt)
{
	if(rtc_dev){
...
bool_t valid_datetime(TDateTime* dt)
{
	if(dt){
...
bool_t datetime_unzip(unsigned int zip_value,TDateTime *dt)
{
	TZipDateTimeDef zdt;
	zdt.value = zip_value;
...
bool_t register_rtc(RTC_Device* dev)
{
	rtc_dev = dev;
